...
@app.get("/reading/toefl", response_model=TextQuestions)
async def get_questions():
    try:
        prompt = "сгенерируй текст на английском языке примерно на 400 слов Темы: Академические текст по различным дисциплинам \
            ...
            правильный вариант ответа на него, а так же объяснение почему именно этот вариант ответа правильный, только объяснение должно быть на русском языке \
...
@app.get("/reading/jlpt", response_model=TextQuestions)
async def get_questions():
    try:
        prompt = "сгенерируй текст на японском языке примерно на 400 слов Темы: Академические текст по различным дисциплинам \
            (естественные науки, социальные науки, бизнес, искусства и т.д.).на примерно такие темы, \
            так же Сгенерируй 10 тестовых вопросов по данному тексту. \
            вопросы на определение лексического значения слов в контексте, нахождение деталей, \
            понимание структуры текста и выделение основной идеи. \
            К каждому вопросу добавь тестовые варианты ответа на него, на японском языке, \
            правильный вариант ответа на него, а так же объяснение почему именно этот вариант ответа правильный, только объяснение должно быть на русском языке \
            в качестве ответа пришли мне json со следующей структурой, чтобы при этом я мог сразу передать его на клиентскую сторону:" + '''
{ 
  "text": "long text in Japanese...", 
  "questionData": [ 
    { 
      "question": "What is the capital of France?", 
      "options": ["Paris", "London", "Berlin", "Madrid"], 
...
@app.get("/listening/toefl", response_model=TextQuestions)
async def get_questions():
    try:
...
@app.get("/listening/jlpt", response_model=TextQuestions)
async def get_questions():
    try:
        prompt = "сгенерируй текст на японском языке примерно на 300 слов Темы: содержание может варьироваться \
            от обсуждения академических тем до повседневных ситуаций, таких как покупка товаров в магазине или обсуждение планов на выходные.\
            Лекции учителей или студентов, интервью с профессионалами, беседы между друзьями и многое другое. \
            То есть это может быть как текст в формате диалога формата (-hi.   -hi, how are you? ...). \
            Так и Академические текст по различным дисциплинам \
...
@app.get("/writing/toefl", response_model=Text)
async def get_questions():
    try:
        prompt = "сгенерируй академический текст на английском языке примерно на 150 слов \
            (естественные науки, социальные науки, бизнес, искусства и т.д.), в тексте не должно быть кавычек \
            в качестве ответа пришли мне json со следующей структурой, чтобы при этом я мог сразу передать его на клиентскую сторону, не присылай мне ничего кроме json:" + '''
{ 
  "text": "long text in English...", 
}
'''
...
@app.get("/writing/jlpt", response_model=Text)
async def get_questions():
    try:
        prompt = "сгенерируй академический текст на японском языке примерно на 150 слов \
            (естественные науки, социальные науки, бизнес, искусства и т.д.), в тексте не должно быть кавычек \
            в качестве ответа пришли мне json со следующей структурой, чтобы при этом я мог сразу передать его на клиентскую сторону, не присылай мне ничего кроме json:" + '''
{ 
  "text": "long text in English...", 
...
@app.post("/writing/check/toefl", response_model=Text)
async def get_questions(request: Check):
    try:
        ...
        - Связность: использование связующих слов и фраз для обеспечения плавности перехода между идеями и абзацами. \
            Так же проверяй все аспекты грамматики, а после пришли подробный отчет обязательно на русском языке по всем моим ошибкам и недочетам, где можно было сделать лучше. \
            в качестве ответа пришли мне json со следующей структурой, чтобы при этом я мог сразу передать его на клиентскую сторону, не присылай мне ничего кроме json: " + '''
{ 
  "text": "long text in English...", 
}
...
@app.post("/writing/check/jlpt", response_model=Text)
async def get_questions(request: Check):
    try:
        prompt = " Вот текст, по которому я писал эссе: (" + request.topic + ") . Вот эссе которое я написал: (" + request.essay + ") Можешь ли ты оценить \
            мое эссе по следующим критериям: - Качество и развитие идей: \
